...
import com.toy.constant.MyConstantUtils;
...
public class ToyUserController {
                             ...
                             HttpSession session) throws Exception
    {
        ...
        pageNum = Integer.parseInt(_page);
        if("".equals(_age) || _age == null) {}
        else
        {
            age = Integer.parseInt(_age);
        ...
        hashMap.put("name", userName);
        hashMap.put("age", age);
        hashMap.put("offset", (pageNum-1) * utils.DEFAULT_PAGE_SIZE);
        hashMap.put("pageSize", utils.DEFAULT_PAGE_SIZE);

        PageHelper.startPage(pageNum, utils.DEFAULT_PAGE_SIZE);
        List<ToyUser> list = userService.selectAll();

        PageInfo page = new PageInfo(list);
        ///
        System.out.println(page);
        modelAndView.addObject("page", page);
        modelAndView.setViewName("user/list1");
        return modelAndView;
    }
    ...
    public ModelAndView add(ModelAndView modelAndView, HttpServletRequest request) throws Exception {
        ToyUser user = null;
    ...
    public ModelAndView update(ModelAndView modelAndView, HttpServletRequest request) throws Exception {
    ...
    public ToyUser getUser(HttpServletRequest request) throws Exception {
